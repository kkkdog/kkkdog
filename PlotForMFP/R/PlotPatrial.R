#' To make patrial plot for MFP
#'
#' This function returns with a nice patrial plot with confidence interval using the results from mfp function or Patrial()
#'
...
#' @seealso PlotPatrial(),mfp package
#' @examples
#'
#' x<-runif(100,0,10)
#' y<-runif(100,0,10)
...
PlotPatrial<-function(data,var="",xlim=data$xlim, ylim=data$ylim,xlab=data$xlab,ylab=data$ylab,log=data$log,
                      shadow=FALSE,shadow.col=rgb(215,215,215,maxColorValue = 255),point=TRUE,
                      p.pch=1,p.col="black",p.cex=1,
                      l.lty="solid",l.col="black",l.cex=1,
                      CI.border=NA,CI.col="grey"){

if("PlotOrNot" %in% names(data)){data=data}else{data=Patrial(data)}
...
if(point==TRUE){points(pa~x,cex=p.cex,data=DataPlot,pch=p.pch,col=p.col)}
}
}
