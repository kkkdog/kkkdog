...
BootCI<-function(data,x,y,B,m,alpha){
  ...
  return(list(PlotForMPF="FitSpline",Mean=MeanSpline,Lower=Lower,Upper=Upper,x=PreX,xin=data[,x],yin=data[,y],LabelX=x,LabelY=y))
}
...
FunCI<-function(data,x,y,alpha,cv){
  ...
  return(list(PlotForMPF="FitSpline",Mean=re$y,Lower=Lower,Upper=Upper,x=re$x,xin=data[,x],yin=data[,y],LabelX=x,LabelY=y))
}
...
LoessCI<-function(data,x,y,alpha,span){
  ...
  return(list(PlotForMPF="FitSpline",Mean=re$fit[order],Lower=Lower[order],Upper=Upper[order],x=data[order,x],xin=data[order,x],yin=data[order,y],LabelX=x,LabelY=y))
}
...
#' confidence interval using bootstrap method. FitSpline returns a list contains initial data, fitted values, and confidence intervals, which can
...
#' temp<-FitSpline(data,x="x",y="y")
#' PlotSpline(temp,col.CI=rgb(70,130,180,maxColorValue = 255),xlab="X",ylab="y")
#' @export
FitSpline<-function(data,x,y,alpha=0.05,cv=TRUE,span=0.75,B=100,m=200,method="loess"){
